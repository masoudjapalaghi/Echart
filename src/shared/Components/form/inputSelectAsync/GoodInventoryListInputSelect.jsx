...
export const GoodInventoryListInputSelect = ({  title = "کالا",hasTitle = true , defaultOptions=true, ...rest }) => {
  const {push} = useRouter()
  const laodPublications = async (inputValue) => {
    try {
      const response = await InventoryProxy.getGoodList(0 , inputValue);
